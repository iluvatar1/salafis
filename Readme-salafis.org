...
edades. Actualmente ([2025-11-25]) se tiene la siguiente configuración:
...
- Solicitud de cuenta :: Usted debe tener cuenta en la sala de física.
  ...
  https://forms.gle/NJDSTN6Ln4tZLHvdA
...
- Instale vscode, y luego instale la extensión remote-explorer o remote-ssh
  (click en la barra vertical a la izquierda en extensiones, busque la
  extensión, instalela)
  #+ATTR_HTML: :width 80%
...
- ~NOMBRECONEXION~: El nombre que le da a la conexion, normalmente es el mismo
  ip interno
- ~IPINTERNO~ : El ip interno al que se desea conectar, es un n\'umero como
  ~192.168.10.X~, donde ~X~ est'a en el rango [2-29] o lo que sea vigente en
  este momento.
- ~USERNAME~: Es su nombre de usuario de la sala de c'omputo.

*** Troubleshooting
Algunas veces windows no soporta directamente e proxy jump y es necesario
conectarse primero al servidor y luego si se permitirá la conexión a un nodo
interno, así que su configuración debe tener las siguientes dos configuraciones:
#+begin_src sh
Host serversalafis 
  Hostname 168.176.35.111 
  Port 443
  User USERNAME
...
Host CONEXIONNAME 
  ...
  User USERNAME
#+end_src
En este caso, se concecta primero al servidor. Y luego sí sería posible
conectarse a un nodo interno. 
...
- Dele click derecho a cualquier host que tenga configurado, y pidale que lo
  ...
  primera vez, acepta). Es probable que le pida dos veces la contrase~na,
  escr'ibala cada vez y de enter. Con esto ya estará conectado al computador
  remoto y podrá usar el explorador de archivos para explorar los archivos
  remotos y editarlos. Tamboen, la terminal se ejecutará remotamente, pero todo
  lo hace desde su computador.
  #+ATTR_HTML: :width 80%
  [[file:figs/vscode-remote-explorer-connection.jpg]]


** Conexión gráfica usando xpra (soporta opengl)
La aplicación xpra, https://xpra.org/index.html, permite realizar una conexión
gráfica que además soporta opengl, de manera que se pueden realizar
visualizaciones con paraview y demás. Usted debe descargar la aplicación para
su sistema operativo, desde  https://github.com/Xpra-org/xpra/wiki/Download , e
instalarla.

Como la coenxión debe realizarse a un cliente interno (no al servidor login), es
necesario que tenga configurado un proxy jump en ssh, que es el protocolo que
usa xpra. Esto es muy sencillo, simplemente edite la configuración de ssh en su
máquina (en linux y macosx es el archivo -~/.ssh/config-, en windows debe buscar la
ubicación exacta ), y para conectarse al cliente interno con ip ~192.168.10.18~,
debe escribir algo como
#+begin_src bash
Host sala18
   HostName 192.168.10.18
   ProxyJump 168.176.35.111:443
   Port 22
   User USERNAME
#+end_src
donde ~USERNAME~ es su nombre de usuario, y el nombre de la conexión es sala18,
en este caso. Luego de esto, ejecute xpra desde la
línea de comando como
#+begin_src bash
Xpra start-desktop --swap-keys=NO --ssh="ssh" ssh:USERNAME@sala18 --exit-with-children --start-child="xfce4-session"
#+end_src
En macosx puede usar ~/Applications/Xpra.app/Contents/Helpers/Xpra~ . En windows
debe buscar la ruta del ejecutable.

Lo anterior le pedirá al menos dos veces la contraseña, una para el servidor
login y la otra para el servidor interno. Acepte cualquier otra confirmación que
le salga.

Si no hay error, debe
abrirse una sesión de escritorio en donde funciona opengl (probado acá con el
comando glxgears)

#+ATTR_HTML: :width 80%
[[file:figs/xpra-connected.png]]

*** Configuración desde la interfaz gráfica
Si ha configurado el acceso ssh al estilo ~passwordless~, puede usar la interfaz
gráfica de xpra para conectarse. Las instrucciones apra esto se encuentran en
toda la red, y basicamente consisten en generar una clave publica y privada con
un comando ~ssh-keygen~, y luego copiar esa clave privada en el servidor con el
comando  ~ssh-copy-īd~. Con esto ya no deberá escribir passwords a siguiente vez
que se conecte. Debe hacer esos dos passos (generación y copia) tanto desde su
máquina hacia el servidor de la unal, como en el servidor de la unal.

NOTA: Al hacer la conexión passwordless, es probable que deba incluir un campo
como
#+begin_src conf
IdentityFile ~/.ssh/MYKEY.pem
#+end_src
en la configuración ssh que está usando. Esto le indica a ssh dónde encontrar la
clave en caso de que no esté instalada en el sitio por defecto.


Ya configurado el acceso passwordless, ejecute la aplicación ~xpra~.
Obtendrá una ventana como la siguiente:
#+ATTR_HTML: :width 40%
[[file:figs/xpra-init.png]]

Luego de click en star, y alli configure la conexion a algún cliente interno.
NOTA: No esta permitido conectarse graficamente al servidor login ya que se
puede sobrecargar.
...
*** Instalacion de xpra en su cuenta de la sala
Por si acaso, si desea instalar su propia version de xrpa, ejecute en su cuenta
#+begin_src bash
CPPFLAGS="-I/usr/include/pygobject-3.0" pip install --user --no-cache-dir xpra
#+end_src

** (DEPRECATED) Conexión gráfica usando x2goclient
REF:
...
** Conexión desde una tableta android
Es posible conectarse desde android a la sala. Lo que se muestra a continución
está probado en android 14. Si su tableta no soporta alguna de las aplicaciones
mostradas, debe encontrar alguna equivalente con la misma funcionalidad. Igual
para iso, debe buscar la alternativa. La idea general es que el dispositivo
móvil se conectará a un servidor VNC o similar que esté ejecutándose en la sala.
Por esta razón, es necesario ejecutar este servidor antes de intentar cualquier
conexión. Los pasos a seguir son:
- Activar el servidor vnc en algún cliente dentro de la sala de cómputo.
- Crear un tunel que redireccione el puerto de ese cleinte a un puerto dentro
  del dispositivo.
- Conectar la aplicación VNC del dispositivo a este puerto.

En este ejemplo se usarán las siguientes aplicaciones para ejecutar en el
dispositivo:
- ssh: juicessh, https://juicessh.com/ (posible alternativa: termius, o termux)
- vnc: AVNC, https://play.google.com/store/apps/details?id=com.gaurav.avnc&hl=es
  o https://f-droid.org/es/packages/com.gaurav.avnc/
*** Initial config en juicessh (only once)
Abra la aplicación y cree una nueva identidad. Diligencia los campos Nickname
(nombre de la identidad), username (nombre de usuario) y password. Luego de
click en el chck arriba a la izquierda.

#+ATTR_HTML: :width 60%
[[file:figs/tablet-juicessh-identity.jpeg]]

Ahora debe crear dos conexiones: una al servidor de login, y la otra al cliente
interno. Para esto, vaya a Connections, y allí debe crear una nueva conexión
(click en el símbolo + en la parte derecha inferior). Allí diligencie:
- Nickname: nombre de la conexión (como ~server unal~ o ~sala18~ )
- Type: SSH
- Address: El ip externo del servidor de la sala (168.176.XXX.XXX), o el ip del
  cliente interno (192.168.10.XX)
- Identity: La identidad que recién creó, con las credenciales de la sala de
  cómputo.
- Port: 443 si es el servidor externo, 22 si es un cliente interno.
- Connect via: Para el servidor login, no coloque nada acá. Para el cliente
  interno, seleccione la conexión que creó para el servidor de la sala.

#+ATTR_HTML: :width 60%
[[file:figs/tablet-juicessh-connection.jpeg]]

Con lo anterior, ya tiene se puede conectar por ssh tanto al servidor de la sala
de cómputo como a un cliente interno con simplemente tocar la conexión que
necesite!
*** Launching the vnc server at the client (every time)
El tunnel se va a crear usando turbovnc en el cliente interno de la sala. Usando
juicessh, entre al cliente interno. Allí, escriba
#+begin_src shell
spack load turbovnc
#+end_src
El siguiente comando solamente se require una vez, en caso de que no lo haya
ejecutado nunca, y sirve para crear un passwrod que asegure su conexión al
servidor gráfico
#+begin_src shell
vncpasswd
#+end_src
Ahora lance el servidor vnc y dbe tomar nota del puerto asignado: Ejecutar
#+begin_src shell
vncserver
#+end_src
Al crear el servidor se asigna un puerto de la forma =590R=. Es importante
...
Cuando termine la sesión, se le recomienda usar el comando
#+begin_src
vncserver -kill :R
#+end_src
o estará saturando el computador y tendra problemas futuros para conectarse.

#+ATTR_HTML: :width 90%
[[file:figs/tablet-juicessh-ssh.jpeg]]

*** Connecting to the vnc server from the tablet/cellphone
**** Connection config (once)
Abar AVNC y configure una nueva conexión. Allí coloquele un nombre, coloque el
ip del cliente interno y el puerto, que debe ser 590R donde R es el número que
extrajo anteriormente. Note que si este número cambia, debe editar esta
configuración. Seleccione ssh tunnel, coloque el ip del servidor
externo, el puerto 443, seleccione password y coloque el password de la sala.

#+ATTR_HTML: :width 60%
[[file:figs/tablet-AVNC-config.jpeg]]

**** Connection
Simplemente de click en la configuración. Ya estará conectado a la sala y con
soporte de opengl! Si le pide password, use el que escribión cuando ejecutó
vncpassword. Lo único que debe tener en cuenta es que si cambia el número
R del servidor vncserver, debe cambiarlo en la configuración.
#+ATTR_HTML: :width 60%
[[file:figs/tablet-AVNC-connection.jpeg]]
...
De todas maneras, otra forma sencilla es simplamente compartir la conexión de su
celular con su computador. Evidentemente el celular debe tener conexión de
datos. Pero si prefiere hacerlo por la red de la unal, tendrá que seguir lo que
